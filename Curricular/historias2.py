...
basura = {
    ...
    9: 'jugalindog el Friday, December 19, 2025 7:57 PM',
    10: 'Créditos HAP'
...
for archivo in os.listdir(CARPETA_PDFS):
    ...
    Semestre_inicio_match = re.search(r'Periodo admisión:\s*(\d{4}-[12]S)', texto)
...
#                print(detalles)
                ...
                asignatura_valida = (
                    nombre_asig in malla_curricular or
                    nombre_asig in optativas_produccion or
                    nombre_asig in asignaturas_extra or
                    nombre_asig in asignaturas_posgrado
                )
                
                if asignatura_valida:
                    datos.append({
                        'nombre': nombre,
                        'documento': documento,
                        'plan': plan,
                    ...
                    print(f"ℹ️ Asignatura no reconocida (omitida): {nombre_asig} ({codigo}) - Estudiante: {nombre}")
            j += 1


# Exportar a Excel
#df = pd.DataFrame(datos)
#df.to_excel("Historias_academicas_2.xlsx", index=False)
