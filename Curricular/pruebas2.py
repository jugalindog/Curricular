        ...
        while j < len(lineas):
            ...
            if re.fullmatch(r'\((\d{6,7}(?:-B)?)\)', actual):
                ...
                if nombre_encontrado:
                    # Unir nombre con código - SIMPLE: reemplazar la línea en lineas_unidas directamente
                    nombre_limpio = nombre_encontrado.strip()
                    lineas_unidas[idx_nombre] = f"{nombre_limpio} ({codigo})"
                    ...
                    if not any(p in nombre_candidato.lower() for p in encabezado_claves) and es_nombre_valido(nombre_candidato):
                        if lineas_unidas and lineas_unidas[-1].strip() == nombre_candidato:
                            lineas_unidas[-1] = f"{nombre_candidato} ({codigo})"
                        else:
                            lineas_unidas.append(f"{nombre_candidato} ({codigo})")
                        j += 1
                        continue
                    else:
...
#                print(detalles)
                ...
                datos.append({
                    ...
                    'asignatura_truncada': es_nombre_truncado(nombre_asig)  # AUDITORÍA: marcar truncados
                })
            j += 1


# --- EXPORTAR COMO ESTRUCTURA DE DICCIONARIOS POR SEMESTRE ---

# Crear diccionario anidado: {semestre: {asignatura: {detalles}}}
historial_estructurado = {}

for registro in datos:
    semestre = registro['semestre_asignatura']
    
    # Crear el semestre si no existe
    if semestre not in historial_estructurado:
        historial_estructurado[semestre] = {}
    
    # Crear la clave de asignatura (nombre + código)
    asignatura_key = f"{registro['asignatura']} ({registro['codigo_asignatura']})"
    
    # Agregar el registro con todos los detalles solicitados
    historial_estructurado[semestre][asignatura_key] = {
        "Asignatura": registro['asignatura'],
        "Créditos": registro['creditos'],
        "HAP": "",  # Estos datos no están en el PDF extraído
        "HAI": "",  # (podrían estar en detalles si se requiere extraer más)
        "THS": "",
        "Tipología": registro['tipo_asignatura'],
        "Calificación": registro['nota'],
        "Estado": registro['estado'],
        "Anulada": registro['anulada'],
        "N. Veces": 1,  # Asumiendo 1 vez cursada
        "Asignatura_Truncada": registro['asignatura_truncada']  # Auditoría
    }
...
print("\n" + "="*100)
print("HISTORIAL ACADÉMICO ESTRUCTURADO POR SEMESTRE")
print("="*100 + "\n")

for semestre in sorted(historial_estructurado.keys()):
    print(f"\n{semestre} = {{")
    for asignatura, detalles in historial_estructurado[semestre].items():
        print(f'    "{asignatura}": {{')
        for campo, valor in detalles.items():
            if campo != "Asignatura_Truncada":
                print(f'        "{campo}": {json.dumps(valor)},')
        # Auditoría: marcar si está truncada
        if detalles.get("Asignatura_Truncada"):
            print(f'        "⚠️_NOMBRE_TRUNCADO": True')
        print(f'    }},')
    print(f"}}\n")

# Guardar también como JSON para mejor visualización
json_output = json.dumps(historial_estructurado, ensure_ascii=False, indent=2)
with open('Historial_Estructurado.json', 'w', encoding='utf-8') as f:
    f.write(json_output)

print("\n" + "="*100)
print("✅ Archivo JSON generado: Historial_Estructurado.json")
print("="*100)

# También guardar DataFrame con auditoría
df = pd.DataFrame(datos)
df.to_excel("Historias_academicas_con_auditoria.xlsx", index=False)
print("✅ Archivo Excel generado: Historias_academicas_con_auditoria.xlsx")
print(f"\nNota: La columna 'asignatura_truncada' marca asignaturas con nombres cortados por saltos de página")
print(f"Total de asignaturas truncadas detectadas: {df['asignatura_truncada'].sum()}")
