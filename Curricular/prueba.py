...
truncadas = df[df['asignatura_truncada'] == True]
if len(truncadas) > 0:
    print("\n" + "="*100)
    print("⚠️  AUDITORÍA: ASIGNATURAS CON NOMBRES TRUNCADOS DETECTADOS")
    print("="*100)
    print(f"Total: {len(truncadas)} asignaturas\n")
    for idx, row in truncadas.iterrows():
        print(f"  • {row['asignatura']} ({row['codigo_asignatura']}) - Semestre: {row['semestre_asignatura']}")
    print("\n" + "="*100)
